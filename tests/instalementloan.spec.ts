...
test.describe('Instalement loan pop-up', () => {
  test.only('Check instalement pop-up', async ({ page }) => {
    ...
    await page
      ...
      .filter({
        hasText:
          'kredyt ratalny pozostało do spłaty pozostało do spłaty 13 070 ,83 PLN więcej dat',
      })
      .locator('a')
      .click();
    await page
      .locator('i')
      .filter({
        hasText: 'na ten rachunek wpłacaj kwoty rat zgodnie z harmonogramem',
      });
    ...
    // Assert
    await expect(page.locator('#ui-tooltip-22')).toHaveText(
      'na ten rachunek wpłacaj kwoty rat zgodnie z harmonogramem',
    );
